...
import { createEventSchema, updateEventSchema, queryEventSchema } from "../validators/event.validator";
...
export async function getById(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const event = await eventService.findById(req.firmId!, req.params.id as string);
    res.json({ data: event });
  } catch (err) { next(err); }
}
...
export async function remove(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    await eventService.remove(req.firmId!, req.params.id as string);
