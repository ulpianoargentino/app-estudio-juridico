...
Actualización 2026-04-18 (tarea D2.1): rediseño del módulo subexpedientes para hacerlo flexible entre jurisdicciones. Cambios principales: (a) enum `subCaseType` cambia de `PLAINTIFF/DEFENDANT/OTHER` a `EVIDENCE/INCIDENT/OTHER` (Prueba/Incidente/Otro). (b) Drop de columna `sub_case_sequence` (integer auto), add de `sub_case_number` (text, nullable, libre). (c) Tipo y número pasan a opcionales en el formulario; el sub puede crearse vacío. (d) Carátula editable en el form (default = padre). (e) Nuevo endpoint `GET /api/cases/:id/sub-cases/next-number?type=…` que sugiere `{prefijo}{maxN+1}` en base a los `sub_case_number` existentes que matchean el patrón; el front lo muestra como placeholder del input. (f) El número del sub se computa en UI como `{padre.caseNumber}-{sub.subCaseNumber}` o `—` si falta cualquiera. (g) Discriminador del listado principal cambió de `sub_case_type IS NULL` a `NOT EXISTS` sobre `case_links` con `link_type = SUB_CASE` (era bug post-D2.1: subs sin tipo se colaban). Migraciones: `0003_bright_vengeance.sql` (drop) + `0004_green_gwen_stacy.sql` (add). DB estaba limpia, no hubo datos a migrar. Reglas en CLAUDE.md sección "Subexpedientes" (renombrada, ya no dice "estilo Tucumán" exclusivamente).
